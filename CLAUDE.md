...
- `src/core` is pure: no Node, no DOM, no network, no `Date.now()`. Every function takes `now` and `tz`. If you need I/O, you are in the wrong layer.
...
- Autonomous actions go through `Services.intervene` so they land in the ledger with an undo payload. Guardian may defer; it may not scope work down or delete.
